...
/*
 * createPlateGrid({ plates:["hit"|"miss"...], direction:"ltr"|"rtl", hint(HTML文字列, 省略可), readonly, compact, onChange(plates), onDirectionChange(dir) })
  ...
  function render() {
    ...
    html += `<div class="plate-row"><span class="plate-row-label"></span>
      <div class="plate-dir" role="group" aria-label="射撃方向指定">
        <button type="button" class="plate-dir-btn ${direction === 'ltr' ? 'active' : ''}" data-dir="ltr" title="左から右へ撃つ" ${opts.readonly ? 'disabled' : ''}>→</button>
        <span class="plate-dir-label">射撃方向指定</span>
        <button type="button" class="plate-dir-btn ${direction === 'rtl' ? 'active' : ''}" data-dir="rtl" title="右から左へ撃つ" ${opts.readonly ? 'disabled' : ''}>←</button>
      </div></div>`;
